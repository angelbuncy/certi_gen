    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (!Array.isArray(body.items) || body.items.length === 0) return json({ error: "No items" }, 400);
          ...
          for (const item of body.items) {
            if (!item.to || !/.+@.+\..+/.test(item.to)) { errors.push(`Invalid email: ${item.to}`); continue; }
            ...
            if (!res.ok) {
              ...
              errors.push(`${item.to}: ${res.status} ${t.slice(0, 120)}`);
            } else sent++;
          }
